...
#####TEMPTEMPTEMPTEMPTMEP##########TEMPTEMPTEMPTEMPTMEP#####
    ...
    #################################################################################################
        ...
        for i in range(2):
            ...
            All_nch = All_nch/entries
            #All_nch[All_nch==0]=np.nan
            axs[i+2].plot(All_nch,marker='o',linestyle='',label='All NPOMH')
    ...
    #################################################################################################
        ...
        for i in range(2):
            for j in range(12):
                ...
                All_nch = np.asarray([tempS_nch.GetBinContent(k) for k in range(1,Nbins)])
                All_nch = All_nch/entries
                #All_nch[All_nch==10**(-6)]=np.nan
                axs[i+2].plot(All_nch,marker='o',linestyle='',label='NPOMH={}'.format(j))
