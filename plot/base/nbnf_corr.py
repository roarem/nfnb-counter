...
def nbnf(filepath,kin=True):
    ...
    ms = 10        #marker size
    ls = ''        #errorbar linestyle
    lc = 'red'     #fit line color
    ...
    for e,filename in enumerate(filenames):
        ...
        for i in range(25):
            for j in range(25):
                if i or j:
                    tempS_nbnf.Add(f.FindObjectAny(prefix+"NPOM_{:02d}_{:02d}".format(i,j)))
                    tempS_nf.Add(f.FindObjectAny(prefix+"NPOM_NF_{:02d}_{:02d}".format(i,j)))
                else:
                    tempS_nbnf = f.FindObjectAny(prefix+"NPOM_{:02d}_{:02d}".format(i,j))
                    tempS_nf = f.FindObjectAny(prefix+"NPOM_NF_{:02d}_{:02d}".format(i,j))
